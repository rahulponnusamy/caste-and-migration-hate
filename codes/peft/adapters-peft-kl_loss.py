...
def classifier(model_path,output_path,lr):
  ...
  model = AutoModelForSequenceClassification.from_pretrained(model_path, return_dict=True,load_in_8bit=True)
  #####################################################
  #model.set_default_language("ta_IN") #### ONLY for XMOD
  #####################################################
  model = get_peft_model(model, peft_config)
  #model.print_trainable_parameters()
  model.add_adapter(peft_config = peft_config, adapter_name = "adapter_1")
...
#######################################
lr = 4e-5
root = os.path.join("adapters-peft-bceloss",str(lr))
sub_dir ="bloom"
